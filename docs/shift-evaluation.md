...
Re-run against `runs/train/patience-{arch}/best` — `--patience 5 --epochs
60` (py#38) in place of the fixed 16-epoch budget the table above used.

| model | shallow (4-6) | deep (7-12) | all | value MAE | value sign |
|---|---|---|---|---|---|
| `cpool-c191-b6` | 0.9219 | **0.9953** | **0.9626** | **0.0646** | **0.9690** |
| `attn-d192-b6` | 0.8905 | 0.9928 | 0.9472 | 0.0694 | 0.9664 |
| `resnet-c128-b6` | 0.8905 | 0.9850 | 0.9429 | 0.0908 | 0.9597 |
| `mlp-h455-b4` | 0.8843 | 0.9700 | 0.9318 | 0.1215 | 0.9503 |

Ranking is unchanged, and deep/overall/value all improved for every model
with the extra epochs. **Shallow accuracy did not.** It fell for `resnet`
(0.9126 → 0.8905), `cpool` (0.9295 → 0.9219) and `attn` (0.9102 → 0.8905),
and was flat for `mlp`. Plies 4-6 hold zero or near-zero training positions
(`runs/coverage.md`), so the extra epochs bought more of the mid-game
distribution the corpus is actually dense in, not the extrapolation regime
this document exists to measure — training longer is not a free win on
this number. Regenerated by `scripts/evaluate_lineup.sh`, output
